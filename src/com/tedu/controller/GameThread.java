...
//			gameElementPK(fire,map);
			gameTime++;//唯一的时间控制
			if(boss.isEmpty()) {
				GameLoad.setIsOver(true);
				if((GameLoad.getIsOver() && die.isEmpty() && enemy.isEmpty()) || play.isEmpty()) {
					break;
				}
			}
			try {
...
//		this.isOver = true;
	...
	public void gameElementRemoveAll(Map<GameElement, List<ElementObj>> all) {
		for(GameElement ge : GameElement.values()) {
			List<ElementObj> list = all.get(ge);
			//编写这样的直接操作集合数据的代码建议不要使用迭代器
			//for(int i=list.size()-1; i>=0; i--) {
